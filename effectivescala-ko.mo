...
`Seq`, `Set`, `Map` 중 하나로 압축된다. 메서드에 시퀀스가 필요하다면,
`List[T]`가 아닌, `Seq[T]`를 사용한다.
...
제공한다. `for`는 여러 시퀀스를 단순하게 할 때 특히 유용하다.
